...
@Component
    public class AuthGlobalFilter implements GlobalFilter {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            ...
            // 验证登录节点 trade.html,myOrder.html,list.html 用","截取
            for (String authUrl : authUrls.split(",")) {
                ...
                if (path.indexOf(authUrl) != -1 && StringUtils.isEmpty(userId)) {
                    ...
                    response.getHeaders().set(HttpHeaders.LOCATION, "http://www.omall.com/login.html?originUrl=" + request.getURI());
                    // 重定向到登录
    ...
    private String getUserId(ServerHttpRequest request) {
        ...
        if(!StringUtils.isEmpty(token)) {
            ...
            String ip = userJson.getString("ip");//旧IP
            String curIp = IpUtil.getGatwayIpAddress(request);//新IP
            //校验token是否被盗用
            if(ip.equals(curIp)) {
